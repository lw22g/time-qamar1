...
async function loadEmployeeInfo() {
  try {
    ...
    const logsRes = await fetch('/api/attendance/my-logs');
    const logs = await logsRes.json();
    renderEmployeeAttendanceHistory(logs);

    const notifRes = await fetch('/api/notifications/my-notifications');
...
async function loadAdminStats() {
  try {
    ...
    const logsRes = await fetch('/api/admin/attendance');
    const logs = await logsRes.json();
    ...
    const deviceRes = await fetch('/api/device/status');
    const devStatus = await deviceRes.json();
    document.getElementById('admin-stat-devices').innerText = `${devStatus.deviceCount} / ${devStatus.maxDevices}`;
...
async function loadAllEmployees() {
  try {
    ...
    allEmployeesCached = await res.json();
...
async function handleEmployeeFormSubmit(e) {
  ...
  try {
    const res = await fetch(url, {
      method: method,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, username, password, shift_start, shift_end })
    });
    const data = await res.json();
...
async function handleSendNotification(e) {
  ...
  try {
    const res = await fetch('/api/admin/notifications', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId, type, message, amount })
    });
    const data = await res.json();
...
async function revokeDevice(token, name) {
  ...
  try {
    const res = await fetch(`/api/admin/devices/${token}`, { method: 'DELETE' });
    const data = await res.json();
